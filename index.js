...
const app = dialogflow({debug: true, clientId: '357385871185-sli916pfn6g246g13ogs6nahe9dk6v7m.apps.googleusercontent.com',});

//incluso pra teste de captura de email
/* app.intent('Autentica Usuario', (conv, input,signin) => {
  if (signin.status === 'OK') {
...
app.intent('Seleciona Opcao', (conv, params, option) => {
  if (!option) {
    ...
    app.intent('Solicita Permissao', (conv, params, permissionGranted) => {
      if (!permissionGranted) {
        throw new Error('Permissão negada');
      }
      conv.ask(`<speak><prosody rate="112%" pitch="-1.5st"><p><s>Se quiser encontrar uma agência, diga a cidade e UF <break time="600ms" />, bairro ou número da agência.</s><s> Se quiser algum terminal mais próximo de você, diga  <emphasis level="moderate"> "terminal" </emphasis></s></p></prosody></speak>`)
